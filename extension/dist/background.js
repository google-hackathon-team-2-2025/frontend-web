...
async function cropImage(imageDataUrl, selection) {
  try {
    ...
    const height = Math.max(1, Math.min(selection.height, imageBitmap.height - y));
    ...
    const croppedBlob = await canvas.convertToBlob({ type: 'image/png', quality: 0.9 });
...
async function handleAreaCapture(request, sender) {
  try {
    ...
    const fullScreenshot = await chrome.tabs.captureVisibleTab(sender.tab.windowId, {
      format: "png",
      quality: 90,
    });

    console.log("Screenshot captured, selection coordinates:", request.selection);
