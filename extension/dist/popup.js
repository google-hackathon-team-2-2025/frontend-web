...
async function handleAreaSelection() {
  try {
    ...
    if (tab.url.startsWith('chrome://') || tab.url.startsWith('chrome-extension://') || tab.url.startsWith('edge://') || tab.url.startsWith('about:')) {
      throw new Error('Cannot access browser internal pages. Please try on a regular website.');
    }
...
function createSelectionOverlay() {
  ...
  instruction.style.cssText = `
    ...
    z-index: 1000000;
  `;
  instruction.textContent = 'Click and drag to select an area to fact-check • Press ESC to cancel';
